...
def train(config, model, device, dataloader, valdataloader, num_of_train, num_of_val):
    run_name = str(config["pathing"]["run_date"])
    ...
    n_train = num_of_train * batch_size
    n_val = num_of_val
    ...
    for epoch in range(starting_epoch, total_epochs):
        ...
        for i, data in pbar:
            idx, direction, volume, segmentation = data
        ...
        if epoch % 5 == 4:
            ...
            dice = tot / n_val
            savepath = model_save_dir + str(run_name) + '/' + "Epoch{}_LS-{:.3f}_DC-{:.3f}.pth".format(epoch,
                                                                                                       train_loss, dice)
